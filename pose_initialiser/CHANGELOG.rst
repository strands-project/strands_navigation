...
* * Fixing CMakeLists, Package.xml and License for pose initialiser
  ...
  * Adding warning when monitored_navigation has not started on topological_navigation
  * '/current_node' and '/closest_node' are now latched topics that are only published
  when changed except if '~OnlyLatched' parameter is set to false when they
  will be published with a frequency depending on '~LocalisationThrottle' value
* Recreated directory
