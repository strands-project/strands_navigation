...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print ("Usage: remove_node_tags.py <in_file> <out_file> <config_file>")
    else:
        in_file = sys.argv[1]
        out_file = sys.argv[2]
        cfg_file = sys.argv[3]
        ...
        for node in topo_map:
            if node["meta"]["node"] in node_tag:
                if "tag" in node["meta"]:
                    if node_tag[node["meta"]["node"]] in node["meta"]["tag"]:
                        node["meta"]["tag"].remove(node_tag[node["meta"]["node"]])
                        if not node["meta"]["tag"]:
                            node["meta"].pop("tag")

        with open(out_file, 'w') as f_out:
            yaml.dump(topo_map, f_out, default_flow_style=False)
        print("Successfully created the new file removing configured node tags")
